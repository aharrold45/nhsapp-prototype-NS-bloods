...
function suggestedHoursForDay(key, clinicId, open, close, dayOffset, clinicIndex) {
  ...
  // Evening hours only appear from day 4 onwards — in days 1–3 fringe is
  // early-morning only, so extended-hours clinics don't show evening too soon.
  const fringe = dayOffset < 4
...
function generateFullAvailability(postcodeKey, clinicId, openingHours, numDays, isClosest) {
  ...
  if (isClosest) allDayHours = stripFirstAndLast(allDayHours);
  const clinicIndex = isClosest ? 0 : 1;

  const days = [];
  for (let p = 0; p < numDays; p++) {
    const meta = dayMeta(p);
    // Day 0: match the suggested times box exactly.
    // Days 1–3: no evening slots yet, even for extended-hours clinics.
    // Day 4+: full hours including evening where the clinic supports it.
    const dayHours = p === 0
